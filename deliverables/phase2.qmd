...
# read data from disk
eod_data = pd.read_parquet('/home/noahg/MATH316/project2/data/0004691-251025141854904.parquet')
...
## Activity Habits
```{python}
birdwatcher_checklists = eod_data.groupby(['recordedBy', 'eventYear'])['checklist_id'].nunique().sort_values(ascending=False)
...
birdwatcher_checklists_pivot.head(n=10).drop(
    columns=['Total Checklists', '% of All Checklists']).plot.barh(
        stacked=True, figsize=(10, 6), title='Checklists by Birdwatchers per Year')
```

- 39,222 birdwatchers, over 1.4 million checklists
    - Multiple birdwatchers can be associated with same checklist
- obsr501068 produced 6,293 checklists (0.43% of all checklists)
    - No clear overall major contributors

## Seasonal Habits
```{python}
checklists_by_observer_season_year = eod_data.groupby(['recordedBy', 'eventYear', 'eventSeason'])['checklist_id'].nunique().reset_index()
checklists_by_observer_season_year_pivot = checklists_by_observer_season_year.pivot_table(index=['recordedBy', 'eventSeason'], columns='eventYear', values='checklist_id', fill_value=0)

# calculate total checklists per season per birdwatcher
checklists_by_observer_season_year_pivot['Total Birdwatcher Checklists'] = checklists_by_observer_season_year_pivot.sum(axis=1).fillna(0)

# calculate total checklists per birdwatcher across all seasons/years
observer_checklist_totals = eod_data.groupby(['recordedBy'])['checklist_id'].nunique()

# calculate percentage of checklists per season for each birdwatcher
def calculate_seasonal_checklists_pct(row):
    observer = row.name[0] # get recordedBy from MultiIndex
    total_checklists_by_observer = observer_checklist_totals.get(observer, 0)
    if total_checklists_by_observer > 0:
        return round((row['Total Birdwatcher Checklists'] / total_checklists_by_observer) * 100.0, ndigits=3)
    else:
        return 0.0
checklists_by_observer_season_year_pivot['% of Birdwatcher\'s Total Checklists'] = checklists_by_observer_season_year_pivot.apply(calculate_seasonal_checklists_pct, axis=1)

checklists_by_observer_season_year_pivot.sort_values(by='Total Birdwatcher Checklists', ascending=False).head(n=10).to_html()
```
- Spring and Summer seem to be most active seasons for birdwatchers
...
- Individual birdwatchers can have a significant impact on the number of checklists for a country but not overall
    - obsr626844 contributed >%56 of Curaçao's checklists
:::
...
frontalis_chart
```
- Goal: forecast sightings for Pyrrhura Frontalis (maroon-bellied conure) over next 6 months
